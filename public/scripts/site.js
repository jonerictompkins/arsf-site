...
const legacyRemembranceLinks = document.querySelector("#legacy-remembrance-links");

if (legacyRemembranceLinks) {
  try {
    const memorialId = new URLSearchParams(window.location.search).get("memorial");
    const links = JSON.parse(legacyRemembranceLinks.textContent);
    if (memorialId && links[memorialId]) {
      window.location.replace(links[memorialId]);
    }
  } catch {
    // The shared remembrance remains complete if an old link cannot be redirected.
  }
}
